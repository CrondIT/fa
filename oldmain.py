...
app.add_middleware(
    ...
    allow_origins=[
        "*"
    ],  # Разрешить запросы с любых доменов (можно указать конкретные домены)
    allow_credentials=True,
    allow_methods=["*"],  # Разрешить любые методы (GET, POST, PUT, и т.д.)
...
def add_variable(variable: Variable, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=400, detail="Переменная с таким ключом уже существует"
        )
...
def get_variable(key: str, db: Session = Depends(get_db)):
    ...
    variable = (
        ...
        .filter(EnvironmentVariable.key == key)
        .first()
    )
    if not variable:
